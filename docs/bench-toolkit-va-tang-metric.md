...
> file này được giữ lại như reference về cách bóc metric và thiết kế A/B bench.
> Nhiều ví dụ bên dưới gắn với setup cũ `Qwen3.5-2B + trace-round1 + scripts/gpu-l40s-bench`.
> Nếu tái dùng cho ruleset hiện tại thì đổi sang `LFM2.5-1.2B-Instruct`,
> `data/trace_grading_public.jsonl`, và bộ `bench/` + `scripts/` hiện tại trong repo.
