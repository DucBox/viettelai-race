...
> Ghi chú cập nhật ngày 2026-07-17:
> file này được giữ lại như một checklist hạ tầng / bench reference.
> Nếu dùng lại cho ruleset hiện tại thì thay các tham chiếu cũ
> `Qwen/Qwen3.5-2B`, `trace-round1.jsonl`, `scripts/gpu-l40s-bench/`
> bằng `LiquidAI/LFM2.5-1.2B-Instruct`, `data/trace_grading_public.jsonl`
> và bộ script/bench hiện tại của repo.
...
Copy bộ script bench hiện tại + trace hiện tại + model lên máy mới:

```bash
scp -r bench scripts data/trace_grading_public.jsonl serve/models/lfm2.5-1.2b <host>:/root/
```
...
**Serve configs** (legacy example, cần đổi theo model/config hiện tại):
| File | Nội dung |
|---|---|
...
| `serve_specdecode.sh` | v12 + speculative config cũ. Chỉ giữ để tham khảo pattern A/B. |
...
| File | Nội dung |
|---|---|
| `replay_trace_detailed.py` | **Công cụ đo chính.** Khi tái dùng cho ruleset mới, đổi input sang trace hiện tại và model hiện tại. |
