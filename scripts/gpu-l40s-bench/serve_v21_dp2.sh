...
nohup /usr/bin/python3 -m vllm.entrypoints.openai.api_server \
  ...
  --max-model-len=48000 --gpu-memory-utilization=0.185 --tensor-parallel-size=1 \
  --enable-prefix-caching --language-model-only --kv-cache-dtype=fp8 \
  --calculate-kv-scales --max-num-seqs=32 --quantization=fp8 \
  --gdn-prefill-backend=flashinfer --max-num-batched-tokens=2048 \
  > /root/vllm_serve_v21.log 2>&1 &
echo $! > /root/vllm.pid
