#!/bin/bash
# A/B 3 lever THAT (sau khi source-trace loai xxhash[vo]/kv-sharing[no-op]):
#   base   : hien tai (fp8 seqs20 batch3216)
#   rend8  : --renderer-num-workers=8   -> parallel tokenize (tan cong frontend_prep)
#   align  : --mamba-cache-mode=align   -> cache GDN state 18/24 layer (tan cong QUEUE round2-6)
# Tat ca +--mm-processor-cache-gb=0 (bat buoc cho renderer>1; vo hai vi text-only) de dong nhat env.
# Fully-instrumented + reconcile moi rep. Interleaved cold, cung session.
set -u
...
CONFIGS="${CONFIGS:-base rend8 align}"; REPS="${REPS:-3}"
...
extra_for() {
  case "$1" in
    ...
    align) echo "--mamba-cache-mode=align" ;;
  esac
}
...
run_one() {
  ...
  for i in $(seq 1 400); do
    ...
    kill -0 "$(cat /root/vllm.pid 2>/dev/null)" 2>/dev/null || { echo "[FAIL proc chet]"; grep -iE "error|valueerror|traceback" "$OUT/${tag}_serve.log" | head -4; return 1; }
    sleep 1
  done
  ...
  # canh bao neu server co loi preprocess (nhu xxhash vo truoc day)
  local nerr; nerr=$(grep -c "Unexpected error pre-processing" "$OUT/${tag}_serve.log" 2>/dev/null)
  [ "$nerr" -gt 0 ] && echo "  ⚠️ $tag co $nerr loi preprocess -> KHONG tin"
  ...
  echo "[$tag]"; grep -E "TTFT [0-9]|queue  |prefill  |residual  |frontend_prep|TPOT" "$OUT/${tag}_recon.txt" 2>/dev/null | head -6
}

echo "########## A/B levers [$CONFIGS] x $REPS rep interleaved ##########"
for rep in $(seq 1 $REPS); do
  for c in $CONFIGS; do run_one "$c" "$rep"; done
done
kill_server
echo "AB_LEVERS_DONE -> $OUT"
